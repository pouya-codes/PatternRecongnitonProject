...
def convert_mask_data(mask, resolution = RESOLUTION, from_background_color = COCO_BACKGROUND,
    ...
    '''
    ##Not needed mask = image_resize2square(mask, resolution)
...
def custom_background(img):
    #To change the background of the cell segmentation to use it on segcaps
    mask = cv2.subtract(255, img)
    if GRAYSCALE == True:
        # Only need one channel for black and white
        #mask = mask[:,:,:1]
        mask = mask.reshape(mask.shape[0],mask.shape[1],1)
    ...
    # mask[mask != 1] = 255 # Non Person / Background
    mask = mask.astype(np.uint8)
